...
from core.trainer import Trainer, unsupervise_Trainer
from dataset import DATALOADER_REGISTRY, UNSUPERVISE_DATALOADER_REGISTRY
...
def main_worker(rank, world_size, dist_url, final_output_dir, cfg):
    ...
    # Data loading code
    real_loader, camera_loader = UNSUPERVISE_DATALOADER_REGISTRY.build(cfg)
    ...
    if cfg.AUTO_RESUME:
        if cfg.RESUME_FILE != '':
            checkpoint_file =  cfg.RESUME_FILE
        else:
        ...
        if os.path.exists(checkpoint_file):
            ...
            model.module.init_model(checkpoint_file)
    ...
    is_iter=cfg.is_iter
    if is_iter:
        trainer = unsupervise_Trainer(cfg, model, rank, final_output_dir,logger=logger,lr_scheduler=lr_scheduler)
    else:
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        trainer.train(epoch, real_loader, camera_loader, optimizer)
        ...
        if (not cfg.DDP and epoch%(cfg.TRAIN.SAVE_EPOCH_STEP)==0) or (cfg.DDP and rank == 0 and epoch%(cfg.TRAIN.SAVE_EPOCH_STEP)==0) and not cfg.VIS:
            ...
            save_checkpoint({
                ...
                'state_dict': model.module.ema_model.state_dict(),
                'best_state_dict': model.module.ema_model.state_dict(),
                'perf': perf_indicator,
        ...
        if best_model:
            torch.save(
                model.module.ema_model.state_dict(),
    ...
    torch.save(model.module.ema_model.state_dict(), final_model_state_file)
