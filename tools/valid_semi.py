...
def detect():
    ...
    elif opt.DATA == 'real_train':
        result_dir = 'results/eval_real_train'
        file_path = 'Real/train_list.txt'
    ...
    for path in tqdm(img_list):
        ...
        if not use_gt_mask:
            ...
            poses=np.zeros((gts['scales'].shape[0],4,4))
            poses[:,3,3]=1
            poses[:,:3,:3]=gts['rotations']*(gts['scales'][...,None,None])
            poses[:,:3,3]=gts['translations']
            gts['poses']=poses
